...
describe("app", () => {
  describe("api", () => {
    test("GET 200: responds with a JSON object of all the available endpoints", () => {
      return request(app)
        .get("/api")
        .expect(200)
        .then(({ body }) => {
    ...
    test("GET 200: orders the topic by the specific sort by query", () => {
      return request(app)
        .get("/api/articles?sort_by=created_at&&topic=cats")
        .expect(200)
        .then(({ body }) => {
